 ...
 * get_prefix_2() - 
 * 
 * copy from get_prefix_1() and only used for tc command 
 * 
 * Why I add this function instead of get_prefix_1() for tc.
 * This is because when I enable ELBOX_PROGS_GPL_TC_U32_IP_PORT_RANGE and
 * use a command like below:
 *
 * # ip route add 172.18.69.6 via 172.18.75.254 dev eth2.2
 * RTNETLINK answers: Invalid argument
 * 
 * I can't add this static route successfully.
 * So, to resolve this issue, I just add a new function called get_prefix_2() instead of get_prefix_1(),
 * this function is only used for tc command not for ip route command.
 * 
 */
int get_prefix_2(inet_prefix *dst, const char *arg, int family)
{
	int err;
	unsigned plen;
	char *slash;
