...
function isblocked($channel, $freq)
{
	...
	//1.To verify if the file is existed or not.
	if(isfile("/var/dfs_blocked.txt") != 1) {return 0;}
	...
	while ($idx < $count)
	{
		...
		if ($p!="")
		{
			...
			if ($active=="1" && $wifi!="")
			{
				...
				if ($auth == "OPEN")			
				{
					if($encrypt == "WEP") {$authtype = $authtype.$delimiter."WEPAUTO";}
					else {$authtype = $authtype.$delimiter."OPEN";}
				}
				else if ($auth == "SHARED")			{$authtype = $authtype.$delimiter."SHARED";}
				else if ($auth == "BOTH")			{$authtype = $authtype.$delimiter."WEPAUTO";}
...
$phy_G  = XNODE_getpathbytarget("", "phyinf", "uid", "WLAN-1.2");
/* ----------------------------- get configuration -----------------------------------*/
/* country code */
...
else if ($wlmode == "n")	
{
	...
	$bsc_rate = "15";	$wmm_param = "g"; $en11n = 1; $only11N = 1;
}
...
if($freq=="5") {$channel = query($phy."/media/channel_Aband");}
else {$channel = query($phy."/media/channel");}
...
$guardinterval	= query($phy."/media/dot11n/guardinterval");
$txpower	= query($phy."/media/txpower");
$coexistence = query($phy."/media/coexistence/enable");
...
if($coexistence!="")
{
    echo  "HT_BSSCoexistence=".$coexistence."\n";
}
/**********************************************************************************************
...
{
	if (query($phy."/media/dot11n/bandwidth_Aband") == "20")	{$bw = 0;}
	else														{$bw = 1;}
}	
...
if ( query($phy."/media/maxstanum/enable") == "1" )
{
	$maxstanum = query($phy."/media/maxstanum/maxstanum");
	echo "MaxStaNum=".$maxstanum.";".$maxstanum.";\n";
}
//we set as indoor
...
/*guest zone function*/
$guset_zones_active = query($phy_G."/active");
if($guset_zones_active == "1")
{
	...
	{
		echo "NoForwarding=0;1;\n";
	}
	else
	{
		echo "NoForwarding=0;0;\n";
	}		
