...
function inet_ipv4_dynamic($inf, $ifname, $inet, $inetp)
{
	...
	/* check ip alias, if have, then start netbios and llmnr service for alias ip */
	$stsp = XNODE_getpathbytarget("/runtime", "inf", "uid", $inf, 0);
	$cnt = query($stsp."/ipalias/cnt")+0;
	if($cnt!="" && $cnt!="0") 	{	startcmd("service NAMERESOLV.".$inf." restart"); 	}
	/*for dhcp auto ip*/
	$autoip=query($stsp."/ipalias/ipv4/autoip");
	if($autoip!="")
	{
		startcmd("ip addr add ".$autoip."/16 broadcast 169.254.255.255 dev ".$ifname."");
		startcmd("service MDNSRESPONDER restart");
		stopcmd("ip addr del ".$autoip."/16 dev ".$ifname."");
	}
	/* Setup DHCP */
...
//for save just del auto ip
$autoip=query($stsp."/ipalias/ipv4/autoip");
if($autoip!="") { startcmd("ip addr del ".$autoip."/16 dev ".$ifname."");}
