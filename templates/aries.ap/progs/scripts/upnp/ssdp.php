...
/***************************************************************/
/* Same as above functions of Notify for alive/byebye*/
/* It just extra have a ip parameters for httpc as source ip */
/* When need to use those new functions?     */
/* Ans: when interface have multiple ip and shall explicit indicate it's source ip shall be which one */

function SSDP_nt_send_req_by_srcip($phyinf, $max_age, $location, $server, $nts, $nt, $usn , $srcip)
{
	echo "xmldbc -P /etc/scripts/upnp/__NOTIFY.req.ab.php";
	echo " -V \"MAX_AGE="	.$max_age	."\"";
	echo " -V \"LOCATION="	.$location	."\"";
	...
	if ( $srcip != "" )
		echo " | httpc -i ".$phyinf." -d ".$srcip." -d \"239.255.255.250:1900\" -p UDP\n";
	else
		echo " | httpc -i ".$phyinf." -d \"239.255.255.250:1900\" -p UDP\n";
}

function SSDP_nt_send_reqs_by_srcip($times, $phyinf, $max_age, $location, $server, $nts, $nt, $usn, $srcip)
{
	$i = 0;
	while($i < $times)
	{
		SSDP_nt_send_req_by_srcip($phyinf, $max_age, $location, $server, $nts, $nt, $usn, $srcip);
		$i++;
	}
}

function SSDP_nt_walk_all_devices_by_srcip($phyinf, $path, $max_age, $location, $server, $nts, $uuid, $srcip)
{
	foreach ($path)
	{
		/* get device info */
		...
		if ($dtype != "")
		{
			$nt	= $uuid;
			$usn= $uuid;
			SSDP_nt_send_reqs_by_srcip(2, $phyinf, $max_age, $location, $server, $nts, $nt, $usn, $srcip);
			...
			/* walk for embeded devices */
			$child_dpath = $path.":".$InDeX."/deviceList/device";
			SSDP_nt_walk_all_devices_by_srcip($target_host, $phyinf, $child_dpath, $max_age, $location, $server, $nts, $uuid, $srcip);
		}
	}
}
?>
