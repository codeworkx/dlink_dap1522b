...
//	else if	(cut_count($macaddr, "-")==6) $delimiter = "-";		//ip doesn't support the format xx-xx-xx-xx-xx-xx
	...
	if(toupper($macaddr)=="FF:FF:FF:FF:FF:FF" || $macaddr=="01:11:11:11:11:11")
	{
		return 0;	
	}
...
function PHYINF_gettargetmacaddr($mode, $ifname)
{
	...
	if ($mac=="")
	{
		...
		{	
			//Hendry, we are AP!
			/* Only 1 interface, use LAN MAC address */
...
function PHYINF_getmacsetting($ifname)
{
	...
	if ($mac=="")
	{
		if		($ifname=="BRIDGE-1")	$mac = PHYINF_getdevdatamac("lanmac");
		else if	($ifname=="BRIDGE-2")	$mac = PHYINF_getdevdatamac("lanmac");
		else if	($ifname=="BRIDGE-3")	$mac = PHYINF_getdevdatamac("lanmac");
