...
function check_static($path)
{
	...
	if ($gw != "")
	{
		...
		if($ip==$gw)
			return result("FAILED", $path."/gateway", i18n("IPv6 gateway address can not be the same with IPv6 address."));
		$gw_type = INET_v6addrtype($gw);
		if( $gw_type=="ANY" || $gw_type=="MULTICAST" || $gw_type=="LOOPBACK")
			return result("FAILED", $path."/gateway", i18n("Invalid IPv6 gateway address."));
	}
...
function check_ipv6($path, $needgw)
{
	...
	else if	($mode=="STATIC")	$ret = check_static($path);
	...
	foreach ("dns/entry")
	{
		...
		$dns_type = INET_v6addrtype($VaLuE);
		if( $dns_type=="ANY" || $dns_type=="MULTICAST" || $dns_type=="LOOPBACK")
			return result("FAILED", $path."/dns/entry:".$InDeX, i18n("Invalid IPv6 DNS address."));
	}
