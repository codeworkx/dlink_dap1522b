...
function check_dhcp_config($path, $inetp)
{
	...
	$_UID	= cut($_SVC, 1, ".");
	$static = query($inetp."/static");
	if($static == "1")
	{
		$lan_ip		= query($inetp."/ipv4/ipaddr");
		$lan_mask       = query($inetp."/ipv4/mask");
	}
	else
	{
                $lan_ip         = query("runtime/inf/inet/ipv4/ipaddr");
                $lan_mask       = query("runtime/inf/inet/ipv4/mask");

	}
	$lan_network    = ipv4networkid($lan_ip,$lan_mask);
	TRACE_debug("FATLADY: DHCPS4: lan_ip = ".$lan_ip);
	TRACE_debug("FATLADY: DHCPS4: lan_mask = ".$lan_mask);	
	TRACE_debug("FATLADY: DHCPS4: lan_network = ".$lan_network);		
	anchor($path);
	$start_ip = query("start");
	$end_ip = query("end");
	$mask	= query("mask");
	$network = query("network");
	$start = ipv4hostid($start_ip, $mask);
	...
	if (ipv4networkid($start_ip,32)=="")
	{
		set_result("FAILED",$path."/start",i18n("The input start ip address is invalid."));
		return;
	}
	if (isdigit($end_ip) == 0)
	{
	...
	if($lan_network == $network)
	{
		$lan_id = ipv4hostid($lan_ip, $lan_mask);
		if ($start<=$lan_id && $end_ip>=$lan_id)
		{
	...
	/* check router */
	$router = query("router");
	TRACE_debug("FATLADY: DHCPS4: router = ".$router);	
