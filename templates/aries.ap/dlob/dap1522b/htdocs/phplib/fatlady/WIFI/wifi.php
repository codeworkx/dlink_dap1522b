...
{
	$err = 0;
	if ($authtype=="OPEN" || $authtype=="SHARED" || $authtype=="BOTH")
	{
...
function check_wep($p)
{
	...
	if ($err_s > 0)
		return result("FAILED", $p."/wep/size",
				...
				i18n("The WEP key should be 10 or 26 hexadecimal numbers."));
	if ($err_i > 0)
...
function check_radius($p)
{
	...
	if (INET_validv4addr(query($p."/eap/radius"))!=1)
		return result("FAILED", $p."/eap/radius",
				i18n("The IP address of RADIUS is invalid."));
	$val = query($p."/eap/port");
	if ($val > 65535 || $val < 1)
		return result("FAILED", $p."/eap/port",
				i18n("The value of the port should be 1~65535. (recommend port : 1812)."));
	$val = query($p."/eap/secret");
	$len = strlen($val);
	if ($len > 64 || $len < 1)
		return result("FAILED", $p."/eap/secret",
...
function check_psk($p)
{
	...
	$key = query($p."/psk/key");
	$rekey_gtk_interval = query($p."/rekey/gtk");
	$len = strlen($key);
	if ($type == "")
	{
		if ($len>=8 && $len<=63)	$type = 1;
						...
						i18n("The length of the passphrase (PSK) should be between 8 to 64."));
	}
	if ($type==0)
	{
	...
	if ($rekey_gtk_interval!="")
	{
		if (isdigit($rekey_gtk_interval)!=1)
...
function check_media($p)
{
	...
	if ($band=="a") { $wlmode	= query($p."/wlmode_Aband");}
	else 			{ $wlmode	= query($p."/wlmode");}
	...
	/* userlimit */
	$v = query($p."/maxstanum/enable");
	TRACE_debug("FATLADY: limit_enable=".$v);
	if ($v!=1) set($p."/maxstanum/enable", 0);
	else
	{
		$v = query($p."/maxstanum/maxstanum");
		TRACE_debug("FATLADY: limit_value=".$v);
		if (isdigit($v)!=1 || $v<1 || $v>32)
			return result("FAILED", $p."/maxstanum/maxstanum",
						i18n("Invalid value of the User Limit.")." ".
						i18n("The value should be integer and between 1 to 32."));
	}
	...
	/* isc */
	$v = query($p."/w_partition");
	//TRACE_debug("FATLADY: isc=".$v);
	if ($v!=1) set($p."/w_partition", 0);
	/* ewa */
	$v = query($p."/e_partition");
	//TRACE_debug("FATLADY: ewa=".$v);
	if ($v!=1) set($p."/e_partition", 0);
	/* ht2040 */
	$v = query($p."/coexistence/enable");
	//TRACE_debug("FATLADY: ht2040=".$v);
	if ($v!=1) set($p."/coexistence/enable", 0);
	/* preamble */
	$v = query($p."/preamble");
	...
	/* dot11n/bandwidth */
	if($freq=="5") $bandwidth="bandwidth_Aband";
...
function fatlady_wifi($prefix, $phyinf)
{
	...
	if ($base_phyinf=="")
	{
		return result("FAILED","",i18n("The interface is not activated.")." (".$phyinf.")");
	}
	...
	if ($p == "")
	{
		return result("FAILED","",i18n("The wifi node isn't exist.")." (".$wifi_uid.")");
	}
