<div class="orangebox">
	...
	<ul>
		...
		<li><a href="#Wireless"><?echo i18n("Wireless Setup");?></a></li>
		<li><a href="#Network"><?echo i18n("LAN Setup");?></a></li>		
	</ul>
</div>
<div class="blackbox">
...
<? if(query("/runtime/device/switchmode") =="APCLI")	{echo "<!--";} ?>
	<p><?
		echo i18n('If you are new to wireless networking and have never configured a wireless access point before, click "Wireless Network Setup Wizard" and the access point will guide you through a few simple steps to get your wireless network up and running.');
	?></p>
<? if(query("/runtime/device/switchmode") =="APCLI")	{echo "-->";} ?>
<? if(query("/runtime/device/switchmode") !="APCLI")	{echo "<!--";} ?>
	<p><?
		echo i18n('If you are new to wireless networking and have never configured a wireless bridge before, click "Wireless Setup Wizard" and the bridge will guide you through a few simple steps to set up your wireless network.');
	?></p>
<? if(query("/runtime/device/switchmode") !="APCLI")	{echo "-->";} ?>
<!--	<dl>
	...
	<dd>
		...
		<p><?
			echo i18n("The wizard will either display the wireless network settings to guide you through manual configuration, or ask you to press the configuration button on the device. If the device supports Wi-Fi Protected Setup and has a configuration button, you can add it to the network by pressing the configuration button on the device and then the configuration button on the access point within 120 seconds. The status LED on the access point will flash three times if the device has been successfully added to the network.");
		?></p>
		<p><?
...
<div class="blackbox">
	...
	<dl>
		...
		if ($FEATURE_NOSCH!="1")
		{
			echo '<dt>'.i18n("Schedules").'</dt>\n';
			echo '<dd>'.i18n("Select a schedule for when the service will be enabled.").'\n';
			echo i18n("If you do not see the schedule you need in the list of schedules, go to the <a href='tools_sch.php'> MAINTENANCE --> Schedule</a> screen and create a new schedule.").'</dd>\n';
		}
		?>
<? if(query("/runtime/device/switchmode") =="APCLI")	{echo "<!--";} ?>
...
<? if(query("/runtime/device/switchmode") =="APCLI")	{echo "<!--";} ?>
		...
		<dd><?
			echo i18n('Select Visible if you would not like the SSID of your wireless network to be broadcasted by the access point. If this option is Invisible, the SSID of the access point will not be seen by Site Survey utilities, so when setting up your wireless clients, you will have to know the SSID of your access point and enter it manually in order to connect to the access point. This option is disabled by default.');
		?></dd>
<? if(query("/runtime/device/switchmode") =="APCLI")	{echo "-->";} ?>
			...
			<dl>
				...
				<dd><?
					...
					<dl>
						...
						<dl>
							...
							<dd><?
								echo i18n('With this setting both shared and open system are employed. Stations have the option of using either but must still have the correct key to decrypt data.');
							?></dd>
						...
						<dt><?echo i18n('WEP Key Length');?></dt>
						<dd><?
						...
						?></dd>
						<dt><?echo i18n('WEP Key1');?></dt>
						<dd><?
							echo i18n('Keys 1 allow you to easily change wireless encryption settings to maintain a secure network. Simply select the specific key to be used for encrypting wireless data on the network.');
						?></dd>
					</dl>
				</dd>
				...
				<dd><?
					...
					<dl>
                        ...
                        <dd><?
                            echo i18n('Use WPA or WPA2 mode to achieve a balance of strong security and best compatibility. WPA and WPA2 uses different algorithm. WPA-Auto allows both WPA and WPA2.');
                        ?></dd>
...
<? if(query("/runtime/device/switchmode") !="APCLI")    {echo "<!--";} ?>
                                                <dt><?echo i18n('PSK');?></dt>
                                                <dd><?
                                                        echo i18n('When PSK is selected, your wireless clients will need to provide a Passphrase for authentication.');
                                                ?></dd>
<? if(query("/runtime/device/switchmode") !="APCLI")    {echo "-->";} ?>

						<dt><?echo i18n('Pre-Shared Key');?></dt>
						<dd><?
							echo i18n('This is what your wireless clients will need in order to communicate with your access point, enter 8-64 alphanumeric characters. Be sure to write this Passphrase down as you will need to enter it on any other wireless devices you are trying to add to your network.');
						?></dd>
<? if(query("/runtime/device/switchmode") =="APCLI")    {echo "<!--";} ?>
...
<? if(query("/runtime/device/switchmode") !="APCLI")	{echo "-->";} ?>
<? if(query("/runtime/device/switchmode") !="APCLI")	{echo "<!--";} ?>
	<dt><strong><?echo i18n("Wireless Mac Cloning");?></strong></dt>
		<dd>
			<dl>
				<dt><strong><?echo i18n("Enable");?></strong></dt>
				<dd><?echo i18n("If MAC clone function is enabled, before the device forwards a packet which is generated by an Ethernet port station, the source MAC address of the packet will be replaced by the clone MAC address; Otherwise, the packet source MAC address will not be changed.");?></dd>
				<dt><strong><?echo i18n("Mac Source");?></strong></dt>
				<dd><?echo i18n("Auto or manually select the clone MAC address.");?></dd>
				<dt><strong><?echo i18n("Mac Address");?></strong></dt>
				<dd><?echo i18n("The mac address selected by mac clone function.");?></dd>
				<dt><strong><?echo i18n("Scan");?></strong></dt>
				<dd><?echo i18n("The Scan button will help to discover the MAC address of all ethernet port stations.");?></dd>
			</dl>
		</dd>
<? if(query("/runtime/device/switchmode") !="APCLI")	{echo "-->";} ?>
	</dl>
...
<div class="blackbox">
	...
	<dl>
		...
		<dd><?
			...
			<dl>
			<p><?
				echo '<strong>'.i18n('IPv4 Address').'</strong><br/>';
				echo i18n('The IP address of your access point on the local area network. Your local area network settings are based on the address assigned here. For example, 192.168.0.50.');
			?></p>
			<p><?
...
<? if(query("/runtime/device/switchmode") =="APCLI")	{echo "<!--";} ?>	
		...
		<dd>
			<?
			echo i18n('There are several connection types to choose from: Link-local, Static IPv6, Autoconfiguration(SLAAC/DHCPv6). If you are unsure of your connection method, please contact your IPv6 Internet Service Provider.');
			?>
			<dl>
			<p><?
