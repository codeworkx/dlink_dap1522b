...
Page.prototype =
{
		...
		{	
			...
			*/
			...
			if(strlen_SS_ssid > 0)
			{
				...
				{
					orl_SS_ssid=COMM_EscapeHTMLSC(this.SS_ssid[i].substring(0,16));
					cut_first_SS_ssid=COMM_EscapeHTMLSC(this.SS_ssid[i].substring(16,strlen_SS_ssid));
	...
	OnClickConnect: function()
	{
		...
		{	
			...
			window.opener.OBJ("pre_shared_key_sta").style.display = "none";			
		}
		...
		else
		{
			...
			window.opener.OBJ("pre_shared_key_sta").style.display = "block";		
		}
			...
			switch (authtype)
		{
			...
			COMM_SetSelectValue(window.opener.OBJ("security_type_sta"), type);		
			COMM_SetSelectValue(window.opener.OBJ("cipher_type_sta"), cipher);
			COMM_SetSelectValue(window.opener.OBJ("wpa_mode_sta"), wpamode);			
		}
		else
		{  
		...
		{
			COMM_SetSelectValue(window.opener.OBJ("bw_sta"), "20");
			window.opener.OBJ("bw_sta").disabled = true;
		}
		else
		{
			window.opener.OBJ("bw_sta").disabled = true;//false;
		}
