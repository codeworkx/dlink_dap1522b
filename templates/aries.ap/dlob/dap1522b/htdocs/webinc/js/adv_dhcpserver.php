...
Page.prototype =
{
	services: "DHCPS4.BRIDGE-1,RUNTIME.INF.BRIDGE-1",
	OnLoad: function()
	{
		...
		case "OK":
			if (this.ipdirty)
			{
				...
				[
					...
					'<?echo i18n("You can access the device by clicking the link below.");?></a>'	
				];
	...
	PreSubmit: function()
	{
		...
		PXML.IgnoreModule("RUNTIME.INF.BRIDGE-1");
		return PXML.doc;
	},
	IsDirty: null,
	...
	InitLAN: function()
	{
		...
		if (XG(this.inetp+"/addrtype") == "ipv4")
		{
			...
			OBJ("dns").checked	= XG(lan+"/inf/dns4")!="" ? true : false;
		}
		return true;
	},
...
//		var b = this.inetp+"/ipv4";
		...
		var vals = OBJ("startip").value.split(".");
		if (vals.length!=4)
		{
...
*/
	...
	InitDHCPS: function()
	{
		var svc = PXML.FindModule("DHCPS4.BRIDGE-1");
		var inf1p = PXML.FindModule("RUNTIME.INF.BRIDGE-1");
		...
		this.leasep = GPBT(inf1p+"/runtime", "inf", "uid", "BRIDGE-1", false);
		...
		//var start = XG(this.dhcps4+"/start");
		var endipaddr = XG(this.dhcps4+"/end");
		var network = XG(this.dhcps4+"/network");
		var mask = XG(this.dhcps4+"/mask");	
		var netmask = COMM_IPv4INT2MASK(mask);
		//var ipaddr = get_ip(network);
		//var end= COMM_IPv4HOST(endipaddr, mask)
		
		OBJ("dhcpsvr").checked	= (XG(svc+"/inf/dhcps4")!="")? true : false;
		OBJ("startip").value	= XG(this.dhcps4+"/start");
		OBJ("endip").value		= XG(this.dhcps4+"/end");
		OBJ("netmask").value		= netmask;
		OBJ("gateway").value		= XG(this.dhcps4+"/router");		
		OBJ("wins").value		= XG(this.dhcps4+"/wins/entry:1");	
		OBJ("dns").value		= XG(this.dhcps4+"/dns/entry:1");					
	...
	if (XG(svc+"/inf/dhcps4")!="")			// when the dhcp server is enabled show the dynamic dhcp clients list
	{
		...
		for (var i=1; i <= <?=$DHCP_MAX_COUNT?>; i++)
		{
			if (i <= cnt)
			{
				...
				OBJ("ip_"+i).value = COMM_IPv4IPADDR(network, mask, XG(this.dhcps4+"/staticleases/entry:"+i+"/hostid"));
	...
	PreDHCPS: function()
	{
		var lan = PXML.FindModule("DHCPS4.BRIDGE-1");
		...
		var startip = OBJ("startip").value;
		var end = OBJ("endip").value;
		//var ip = get_ip(startip);
		//var start = ip[4];
		var netmask = OBJ("netmask").value;
		var mask = COMM_IPv4MASK2INT(netmask);

		var network = COMM_IPv4NETWORK(startip, mask);		
...
*/			/*
		...
		*/
			...
			XS(this.dhcps4+"/end", end);
			XS(this.dhcps4+"/mask", mask);
			XS(this.dhcps4+"/network", network);
			XS(this.dhcps4+"/router", OBJ("gateway").value);	
			var wins = OBJ("wins").value;
			if(wins!="")			XS(this.dhcps4+"/wins/count", 1);
			else							XS(this.dhcps4+"/wins/count", 0);
			
			XS(this.dhcps4+"/wins/entry:1", OBJ("wins").value);	
			
			var dns = OBJ("dns").value;
			if(dns!="")				XS(this.dhcps4+"/dns/count", 1);
			else							XS(this.dhcps4+"/dns/count", 0);
			
			XS(this.dhcps4+"/dns/entry:1", OBJ("dns").value);		
		...
		for (var i=1; i <= <?=$DHCP_MAX_COUNT?>; i++)
		{
			if (OBJ("en_"+i).checked || OBJ("desc_"+i).value != "" || OBJ("ip_"+i).value != "" || OBJ("mac_"+i).value != "")
			{
				...
				if (TEMP_CheckNetworkAddr(OBJ("ip_"+i).value, network, mask))
				{
					...
					XS(path+"/hostid",		COMM_IPv4HOST(OBJ("ip_"+i).value, mask));
				}
				else
				{
					BODY.ShowAlert("<?echo i18n("Invalid IP address");?>");
		...
		PXML.ActiveModule("DHCPS4.BRIDGE-1");
		return true;
	},
	OnClickDHCPSvr: function()
	{
		if (OBJ("dhcpsvr").checked)
		{
			...
			OBJ("netmask").setAttribute("modified", "false");
			OBJ("gateway").setAttribute("modified", "false");
			OBJ("wins").setAttribute("modified", "false");
			OBJ("dns").setAttribute("modified", "false");
			OBJ("leasetime").setAttribute("modified", "false");
			OBJ("startip").disabled = false;
			OBJ("endip").disabled = false;
			OBJ("netmask").disabled = false;
			OBJ("gateway").disabled = false;
			OBJ("wins").disabled = false;
			OBJ("dns").disabled = false;
		...
		else
		{
			...
			OBJ("netmask").setAttribute("modified", "ignore");
			OBJ("gateway").setAttribute("modified", "ignore");
			OBJ("wins").setAttribute("modified", "ignore");
			OBJ("dns").setAttribute("modified", "ignore");
			OBJ("leasetime").setAttribute("modified", "ignore");
			OBJ("startip").disabled = true;
			OBJ("endip").disabled = true;
			OBJ("netmask").disabled = true;
			OBJ("gateway").disabled = true;
			OBJ("wins").disabled = true;
			OBJ("dns").disabled = true;
