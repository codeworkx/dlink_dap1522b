...
<?
	if ($TEMP_STYLE=="help") echo '\t<link rel="stylesheet" href="/css/support.css" type="text/css">\n';
...
?>
	...
	Body.prototype =
	{
		...
		ShowRebootNeeded: function(banner, msgArray)
		{
			var str = '<h1>'+banner+'</h1>';
		...
		GetCFG: function()
		{
			...
			if (PAGE&&PAGE.services!=null)
			{
				COMM_GetCFG(
					false,
					PAGE.services+",RUNTIME.SWITCHMODE",
					function(xml) {
						PAGE.InitValue(xml);
						PAGE.Synchronize();
...
*/
	...
	/*		if('<?echo $_GLOBALS["TEMP_MYNAME"];?>' != 'bsc_sms_send')
            		{
				if(document.getElementById("whichsubmit") && OBJ("whichsubmit").value === "rule")
				{
                                 var msgArray =  ['<?echo i18n("Please wait");?> ...'];
				}		
				else
				{
           			 var msgArray =
		...
		SubmitCallback: function(code, result)
		{
			...
			{
			case "OK":
				setTimeout('BODY.DelayToShowPage()',3000);
				break;
			case "BUSY":
		...
		{
			var self = this;
			if (isfreset && <?if($_GLOBALS["TEMP_MYNAME"] == "wiz_wlan_br" || $_GLOBALS["TEMP_MYNAME"] == "wiz_wlan_ap"){echo 1;}else{echo 0;}?>)
			{
				if(OBJ("loginusr").value == "") OBJ("loginusr").value = "Admin";
				if (AUTH.AuthorizedGroup < 0)	BODY.LoginSubmit();
			}
		...
		InjectTraTable: function(tblID, uid, data, type)
		{
			var rows = OBJ(tblID).getElementsByTagName("tr");
			var tagTR = null;
			var tagTD = null;
		...
		TurnReboot: function(code, result)
		{
			BODY.ShowContent();
			switch (code)
			{
			case "OK":
				var msgArray = 
					[
						'<p><?echo i18n("Your changes have been saved. The access point must be rebooted for the changes to take effect.");?><br><?echo i18n("You can reboot now, or you can continue to make other changes and reboot later.");?></p>',
						'<p><input type="button" value="<?echo i18n("Reboot Now");?>" onClick="BODY.OnClickReboot();">&nbsp;&nbsp;<input type="button" value="<?echo i18n("Reboot Later");?>" onClick="BODY.OnClickLater();"></p>'
					];
					BODY.ShowRebootNeeded('<?echo i18n("Reboot Needed...");?>',msgArray);
		...
		OnWizReboot: function()
		{
			BODY.Service("REBOOT");
		},
		OnClickLater: function()
		{
				BODY.Service("REBOOT.STATUS");
		...
		{	
			var banner = "Rebooting...";
			var msgArray = ["If you changed the IP address of the AP or wireless client you will need to change the IP address in your browser before accessing the configuration web page again."];
			var delay = 10;
			...
			ajaxObj.onCallback = function (xml)
			{
				...
				{
					if(svc=="REBOOT.STATUS")
					{
							OBJ("rebootitem1").style.display= "";
							OBJ("rebootitem2").style.display= "";
					}
					else
					{	
	...
	var isfreset = <?if (query("/runtime/device/devconfsize")>0) echo 'false'; else echo 'true';?>;
...
<div class="maincontainer">
	...
	<div id="menu" class="topmenucontainer" style="display:none;">
		<div class="<? $switchmode = query("/runtime/device/switchmode"); if ($switchmode == "APCLI") echo "modelname-br"; else echo "modelname-ap"; ?>"><?echo query("/runtime/device/modelname");?></div>
		<div>
			<a<?is_label("basic");?> href="setup.php"><?echo i18n("setup");?></a>
			<a<?is_label("advanced");?> href="advanced.php"><?echo i18n("advanced");?></a>
			<a<?is_label("maintenance");?> href="tools.php"><?echo i18n("maintenance");?></a>
			<a<?is_label("status");?> href="status.php"><?echo i18n("status");?></a>
			<a<?is_label("help");?> href="support.php"><?echo i18n("help");?></a>
		</div>
	</div>
<?
	...
	<div id="login" class="simplecontainer" style="display:none;">
		<div class="simplebody">
			<div class="orangebox">
				...
				<div class="message"><?if(query("/runtime/device/switchmode")=="APCLI") echo i18n("Login to the Bridge"); else echo i18n("Login to the Access Point");?> :</div>
				<div class="loginbox">
				...
				<div id="GAC" class="centerline">
					...
					<tr>
						<td colspan="2" align="center">
							<input type="button" value="<?echo i18n("Login");?>" onClick="BODY.LoginSubmit();" />
						</td>
					</tr>
					</table>
					</center>
...
<div class="copyright">Copyright &copy; 2008-2012 D-Link Corporation. All rights reserved.</div>
</body>
</html>
