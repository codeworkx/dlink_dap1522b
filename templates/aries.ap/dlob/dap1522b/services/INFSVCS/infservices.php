...
function infsvcs_setup($name, $cfg, $sts, $ipv6)
{
	...
	$trafficctrl 	= query("trafficctrl"); //harry_shen add
    $next   = query("infnext");
	$child  = query("child");
	...
	if ($nameresolve>0)	startcmd("service NAMERESOLV."	.$name." restart");
	...
	startcmd("sh /etc/scripts/neaps.sh start ");   //add for support DCC Lite,always run it
	
	if (""!=$bwc) startcmd("service BWC.".$name." restart");
	if (""!=$trafficctrl)		startcmd("service TRAFFICCTRL."		.$name." start"); //harry_shen add
	...
	stopcmd("sh /etc/scripts/neaps.sh stop ");   //add for support DCC Lite
	...
	if (""!=$trafficctrl)		stopcmd("service TRAFFICCTRL."		.$name." stop"); //harry_shen add
...
function infsvcs_lan($index)
{
	...
	{
		stopcmd("service MACCTRL restart");
		//stopcmd("service IPTDEFCHAIN restart");
		stopcmd("service MULTICAST restart");
		stopcmd("service ROUTE.STATIC restart");
		stopcmd("service ROUTE.IPUNNUMBERED restart");
	...
	else
	{
		...
		//startcmd("service IPTDEFCHAIN restart");
	    startcmd("service ROUTE.DYNAMIC	restart\n"); //sandy
		startcmd("service MACCTRL restart");
	}
	startcmd("event INFSVCS.".$uid.".UP");
...
function infsvcs_bridge($index)
{
	...
	{
		stopcmd("servie MACCTRL restart");
		//stopcmd("service IPTDEFCHAIN restart");
		stopcmd("service MULTICAST restart");
		stopcmd("service MACCTRL restart");
	}
