...
#if defined (CONFIG_RT3x52) || defined (CONFIG_RTxx50)
extern u32 is_RT3352;
extern u32 is_RT3052;
extern u32 is_RT5350;
extern u32 is_RT3350;
#ifdef CONFIG_RTxx50
u32 PSE_P2_FC;
u32 GDM_CRC_DROP;
u32 PSE_BUF_DROP;
...
//hendry, we have global variables to record packets/bytes of hw nat. 
#if defined (ALPHA_HW_NAT_STATS)
unsigned long long g_vid1_tx_bytes ;
unsigned long long g_vid2_tx_bytes ;
unsigned long long g_vid1_tx_count ;
unsigned long long g_vid2_tx_count ;
EXPORT_SYMBOL(g_vid1_tx_bytes);
EXPORT_SYMBOL(g_vid2_tx_bytes);
EXPORT_SYMBOL(g_vid1_tx_count);
EXPORT_SYMBOL(g_vid2_tx_count);
#endif
...
#if defined (CONFIG_RALINK_RT3052_MP2) || defined (CONFIG_RT3x52) || defined (CONFIG_RTxx50)
...
void set_fe_pdma_glo_cfg(void)
{
...
#else /* CONFIG_RT3x52 || CONFIG_RTxx50 */
#if defined (CONFIG_RALINK_RT2880) || defined(CONFIG_RALINK_RT2883) || \
    defined (CONFIG_RALINK_RT3052) || defined (CONFIG_RALINK_RT3883)
	//set 1us timer count in unit of clock cycle
	fe_glo_cfg = sysRegRead(FE_GLO_CFG);
	...
	sysRegWrite(FE_GLO_CFG, fe_glo_cfg);
#endif
#endif /* CONFIG_RT3x52 || CONFIG_RTxx50 */
}
...
int forward_config(struct net_device *dev)
{
	...
	if (is_RT5350 == 1) {
		 ...
		 */
		if (is_RT3352 == 1 || is_RT5350 == 1){
			;
		} else if (is_RT3052 == 1 || is_RT3350 == 1) {
...
static int rt2880_eth_setup(struct net_device *dev)
{
	...
	for (i = 0; i < NUM_RX_DESC; i++) {
        ...
        //+++ sync code from raeth/raether.c . bouble: 20110624
		...
		ei_local->rx_ring0[i].rxd_info1.PDP0 = dma_map_single(NULL, skb_put(ei_local->netrx0_skbuf[i], 2), MAX_RX_LENGTH+2, PCI_DMA_FROMDEVICE);
	}
	...
	for (i = 0; i < NUM_RX_DESC; i++) {
		...
		ei_local->rx_ring1[i].rxd_info1.PDP0 = dma_map_single(NULL, skb_put(ei_local->netrx1_skbuf[i], 2), MAX_RX_LENGTH+2, PCI_DMA_FROMDEVICE);
	}
 ...
 */
	...
	if(mcast_rx(rx_skb)==0) {
		...
		rx_ring[rx_dma_owner_idx].rxd_info1.PDP0 = dma_map_single(NULL, skb->data, MAX_RX_LENGTH+2, PCI_DMA_FROMDEVICE);
...
int ei_ioctl(struct net_device *dev, struct ifreq *ifr, int cmd)
{
	...
	esw_reg reg;
#endif
#if defined(CONFIG_RALINK_RT3352) || defined(CONFIG_RALINK_RT5350) || defined (CONFIG_RT3x52) || defined (CONFIG_RTxx50)
        esw_rate ratelimit;
...
#if defined (CONFIG_RALINK_RT3052) || defined (CONFIG_RALINK_RT3352) || defined (CONFIG_RALINK_RT5350)
//joel add keep phy power status as same as bootcode
u32 getphystatus(void)
{
	u32 value=0,portmask=0;
	int i;
	for(i=0;i<5;i++)
	{
		mii_mgr_read(i, 0, &value);
		if(value&0x800)/*pwoer down bit*/
		{
			portmask |=  (1<<i);/*is power down*/
			//printk("%d is power done\n",i);
		}
	}
	//printk("got the power mask is %x\n",portmask);
	return portmask;
}
...
#if defined (CONFIG_RT3052_ASIC) || defined (CONFIG_RT3x52) || defined (CONFIG_RTxx50)
		u32 phy_val2;
#endif

#if defined (CONFIG_RT5350_ASIC)
#if defined (CONFIG_RTxx50)
	if (is_RT5350 == 1)
#endif	
...
// Following line is not to disable VLAN, instad, it add a vlan tag 1. Leon.
...
/* According to Ralink patch patch_sdk3530_20110520_rt3050_phy_setting_update.pdf to optimize phy setting */			
...
//joel add keep phy power status as same as bootcode
	for(i=0;i<5;i++)
	{
		if(powerdownlist&(1<<i))/*down*/
		{
			mii_mgr_write(i, 0, 0x800); /* Disable Lan port */
			//printk("power lan %d down\n",i);
		}	
	}
...
void EthRegDefine(void)
{
	if (is_RT5350 == 1) {
		...
		RAPDMA_OFFSET	= 0x0800;
		SDM_OFFSET	= 0x0C00;

		/* 1. PDMA */
		TX_BASE_PTR0	= (RALINK_FRAME_ENGINE_BASE+RAPDMA_OFFSET+0x000);
		TX_MAX_CNT0	= (RALINK_FRAME_ENGINE_BASE+RAPDMA_OFFSET+0x004);
		TX_CTX_IDX0	= (RALINK_FRAME_ENGINE_BASE+RAPDMA_OFFSET+0x008);
		TX_DTX_IDX0	= (RALINK_FRAME_ENGINE_BASE+RAPDMA_OFFSET+0x00C);

		TX_BASE_PTR1	= (RALINK_FRAME_ENGINE_BASE+RAPDMA_OFFSET+0x010);
		TX_MAX_CNT1	= (RALINK_FRAME_ENGINE_BASE+RAPDMA_OFFSET+0x014);
		TX_CTX_IDX1	= (RALINK_FRAME_ENGINE_BASE+RAPDMA_OFFSET+0x018);
		TX_DTX_IDX1	= (RALINK_FRAME_ENGINE_BASE+RAPDMA_OFFSET+0x01C);

		TX_BASE_PTR2	= (RALINK_FRAME_ENGINE_BASE+RAPDMA_OFFSET+0x020);
		TX_MAX_CNT2	= (RALINK_FRAME_ENGINE_BASE+RAPDMA_OFFSET+0x024);
		TX_CTX_IDX2	= (RALINK_FRAME_ENGINE_BASE+RAPDMA_OFFSET+0x028);
		TX_DTX_IDX2	= (RALINK_FRAME_ENGINE_BASE+RAPDMA_OFFSET+0x02C);

		TX_BASE_PTR3	= (RALINK_FRAME_ENGINE_BASE+RAPDMA_OFFSET+0x030);
		TX_MAX_CNT3	= (RALINK_FRAME_ENGINE_BASE+RAPDMA_OFFSET+0x034);
		TX_CTX_IDX3	= (RALINK_FRAME_ENGINE_BASE+RAPDMA_OFFSET+0x038);
		TX_DTX_IDX3	= (RALINK_FRAME_ENGINE_BASE+RAPDMA_OFFSET+0x03C);

		RX_BASE_PTR0	= (RALINK_FRAME_ENGINE_BASE+RAPDMA_OFFSET+0x100);
		RX_MAX_CNT0	= (RALINK_FRAME_ENGINE_BASE+RAPDMA_OFFSET+0x104);
		RX_CALC_IDX0	= (RALINK_FRAME_ENGINE_BASE+RAPDMA_OFFSET+0x108);
		RX_DRX_IDX0	= (RALINK_FRAME_ENGINE_BASE+RAPDMA_OFFSET+0x10C);

		RX_BASE_PTR1	= (RALINK_FRAME_ENGINE_BASE+RAPDMA_OFFSET+0x110);
		RX_MAX_CNT1	= (RALINK_FRAME_ENGINE_BASE+RAPDMA_OFFSET+0x114);
		RX_CALC_IDX1	= (RALINK_FRAME_ENGINE_BASE+RAPDMA_OFFSET+0x118);
		RX_DRX_IDX1	= (RALINK_FRAME_ENGINE_BASE+RAPDMA_OFFSET+0x11C);

		PDMA_INFO	= (RALINK_FRAME_ENGINE_BASE+RAPDMA_OFFSET+0x200);
		PDMA_GLO_CFG	= (RALINK_FRAME_ENGINE_BASE+RAPDMA_OFFSET+0x204);
		PDMA_RST_IDX	= (RALINK_FRAME_ENGINE_BASE+RAPDMA_OFFSET+0x208);
		PDMA_RST_CFG	= (PDMA_RST_IDX);
		DLY_INT_CFG	= (RALINK_FRAME_ENGINE_BASE+RAPDMA_OFFSET+0x20C);
		FREEQ_THRES	= (RALINK_FRAME_ENGINE_BASE+RAPDMA_OFFSET+0x210);
		INT_STATUS	= (RALINK_FRAME_ENGINE_BASE+RAPDMA_OFFSET+0x220);
		FE_INT_STATUS	= (INT_STATUS);
		INT_MASK	= (RALINK_FRAME_ENGINE_BASE+RAPDMA_OFFSET+0x228);
		FE_INT_ENABLE	= (INT_MASK);
		PDMA_WRR	= (RALINK_FRAME_ENGINE_BASE+RAPDMA_OFFSET+0x280);
		PDMA_SCH_CFG	= (PDMA_WRR);

		SDM_CON		= (RALINK_FRAME_ENGINE_BASE+SDM_OFFSET+0x00);  //Switch DMA configuration
		SDM_RRING	= (RALINK_FRAME_ENGINE_BASE+SDM_OFFSET+0x04);  //Switch DMA Rx Ring
		SDM_TRING	= (RALINK_FRAME_ENGINE_BASE+SDM_OFFSET+0x08);  //Switch DMA Tx Ring
		SDM_MAC_ADRL	= (RALINK_FRAME_ENGINE_BASE+SDM_OFFSET+0x0C);  //Switch MAC address LSB
		SDM_MAC_ADRH	= (RALINK_FRAME_ENGINE_BASE+SDM_OFFSET+0x10);  //Switch MAC Address MSB
		SDM_TPCNT	= (RALINK_FRAME_ENGINE_BASE+SDM_OFFSET+0x100); //Switch DMA Tx packet count
		SDM_TBCNT	= (RALINK_FRAME_ENGINE_BASE+SDM_OFFSET+0x104); //Switch DMA Tx byte count
		SDM_RPCNT	= (RALINK_FRAME_ENGINE_BASE+SDM_OFFSET+0x108); //Switch DMA rx packet count
		SDM_RBCNT	= (RALINK_FRAME_ENGINE_BASE+SDM_OFFSET+0x10C); //Switch DMA rx byte count
		SDM_CS_ERR	= (RALINK_FRAME_ENGINE_BASE+SDM_OFFSET+0x110); //Switch DMA rx checksum error count
	} else {
		//CNT_PPE_AF	= BIT(31); 
		//CNT_GDM_AF	= BIT(29);
		PSE_P2_FC	= BIT(26);
		...
		RAPDMA_OFFSET	= 0x0100;

		/* 1. Frame Engine Global Registers */
		MDIO_ACCESS	= (RALINK_FRAME_ENGINE_BASE+RAFRAMEENGINE_OFFSET+0x00);
		MDIO_CFG 	= (RALINK_FRAME_ENGINE_BASE+RAFRAMEENGINE_OFFSET+0x04);
		MDIO_CFG2 	= (RALINK_FRAME_ENGINE_BASE+RAFRAMEENGINE_OFFSET+0x18);
		FE_GLO_CFG	= (RALINK_FRAME_ENGINE_BASE+RAFRAMEENGINE_OFFSET+0x08);
		FE_RST_GL	= (RALINK_FRAME_ENGINE_BASE+RAFRAMEENGINE_OFFSET+0x0C);
		FE_INT_STATUS	= (RALINK_FRAME_ENGINE_BASE+RAFRAMEENGINE_OFFSET+0x10);
		FE_INT_ENABLE	= (RALINK_FRAME_ENGINE_BASE+RAFRAMEENGINE_OFFSET+0x14);
		MDIO_CFG2	= (RALINK_FRAME_ENGINE_BASE+RAFRAMEENGINE_OFFSET+0x18); //Original:FC_DROP_STA
		FOC_TS_T	= (RALINK_FRAME_ENGINE_BASE+RAFRAMEENGINE_OFFSET+0x1C);


		/* 2. GDMA Registers */
		GDMA1_FWD_CFG	= (RALINK_FRAME_ENGINE_BASE+RAGDMA_OFFSET+0x00);
		GDMA1_SCH_CFG	= (RALINK_FRAME_ENGINE_BASE+RAGDMA_OFFSET+0x04);
		GDMA1_SHPR_CFG	= (RALINK_FRAME_ENGINE_BASE+RAGDMA_OFFSET+0x08);
		GDMA1_MAC_ADRL	= (RALINK_FRAME_ENGINE_BASE+RAGDMA_OFFSET+0x0C);
		GDMA1_MAC_ADRH	= (RALINK_FRAME_ENGINE_BASE+RAGDMA_OFFSET+0x10);

		GDMA2_FWD_CFG	= (RALINK_FRAME_ENGINE_BASE+RAGDMA2_OFFSET+0x00);
		GDMA2_SCH_CFG	= (RALINK_FRAME_ENGINE_BASE+RAGDMA2_OFFSET+0x04);
		GDMA2_SHPR_CFG	= (RALINK_FRAME_ENGINE_BASE+RAGDMA2_OFFSET+0x08);
		GDMA2_MAC_ADRL	= (RALINK_FRAME_ENGINE_BASE+RAGDMA2_OFFSET+0x0C);
		GDMA2_MAC_ADRH	= (RALINK_FRAME_ENGINE_BASE+RAGDMA2_OFFSET+0x10);

		/* 3. PSE */
		PSE_FQ_CFG	= (RALINK_FRAME_ENGINE_BASE+RAPSE_OFFSET+0x00);
		CDMA_FC_CFG	= (RALINK_FRAME_ENGINE_BASE+RAPSE_OFFSET+0x04);
		GDMA1_FC_CFG	= (RALINK_FRAME_ENGINE_BASE+RAPSE_OFFSET+0x08);
		GDMA2_FC_CFG	= (RALINK_FRAME_ENGINE_BASE+RAPSE_OFFSET+0x0C);
		PDMA_FC_CFG	= (RALINK_FRAME_ENGINE_BASE+0x1f0);

		/* 4. CDMA */
		CDMA_CSG_CFG	= (RALINK_FRAME_ENGINE_BASE+RACDMA_OFFSET+0x00);
		CDMA_SCH_CFG	= (RALINK_FRAME_ENGINE_BASE+RACDMA_OFFSET+0x04);
		/* skip ppoe sid and vlan id definition */


		/* 5. PDMA */
		PDMA_GLO_CFG	= (RALINK_FRAME_ENGINE_BASE+RAPDMA_OFFSET+0x00);
		PDMA_RST_CFG	= (RALINK_FRAME_ENGINE_BASE+RAPDMA_OFFSET+0x04);
		PDMA_SCH_CFG	= (RALINK_FRAME_ENGINE_BASE+RAPDMA_OFFSET+0x08);

		DLY_INT_CFG	= (RALINK_FRAME_ENGINE_BASE+RAPDMA_OFFSET+0x0C);

		TX_BASE_PTR0	= (RALINK_FRAME_ENGINE_BASE+RAPDMA_OFFSET+0x10);
		TX_MAX_CNT0	= (RALINK_FRAME_ENGINE_BASE+RAPDMA_OFFSET+0x14);
		TX_CTX_IDX0	= (RALINK_FRAME_ENGINE_BASE+RAPDMA_OFFSET+0x18);
		TX_DTX_IDX0	= (RALINK_FRAME_ENGINE_BASE+RAPDMA_OFFSET+0x1C);

		TX_BASE_PTR1	= (RALINK_FRAME_ENGINE_BASE+RAPDMA_OFFSET+0x20);
		TX_MAX_CNT1	= (RALINK_FRAME_ENGINE_BASE+RAPDMA_OFFSET+0x24);
		TX_CTX_IDX1	= (RALINK_FRAME_ENGINE_BASE+RAPDMA_OFFSET+0x28);
		TX_DTX_IDX1	= (RALINK_FRAME_ENGINE_BASE+RAPDMA_OFFSET+0x2C);

		TX_BASE_PTR2	= (RALINK_FRAME_ENGINE_BASE+RAPDMA_OFFSET+0x40);
		TX_MAX_CNT2	= (RALINK_FRAME_ENGINE_BASE+RAPDMA_OFFSET+0x44);
		TX_CTX_IDX2	= (RALINK_FRAME_ENGINE_BASE+RAPDMA_OFFSET+0x48);
		TX_DTX_IDX2	= (RALINK_FRAME_ENGINE_BASE+RAPDMA_OFFSET+0x4C);

		TX_BASE_PTR3	= (RALINK_FRAME_ENGINE_BASE+RAPDMA_OFFSET+0x50);
		TX_MAX_CNT3	= (RALINK_FRAME_ENGINE_BASE+RAPDMA_OFFSET+0x54);
		TX_CTX_IDX3	= (RALINK_FRAME_ENGINE_BASE+RAPDMA_OFFSET+0x58);
		TX_DTX_IDX3	= (RALINK_FRAME_ENGINE_BASE+RAPDMA_OFFSET+0x5C);

		RX_BASE_PTR0	= (RALINK_FRAME_ENGINE_BASE+RAPDMA_OFFSET+0x30);
		RX_MAX_CNT0	= (RALINK_FRAME_ENGINE_BASE+RAPDMA_OFFSET+0x34);
		RX_CALC_IDX0	= (RALINK_FRAME_ENGINE_BASE+RAPDMA_OFFSET+0x38);
		RX_DRX_IDX0	= (RALINK_FRAME_ENGINE_BASE+RAPDMA_OFFSET+0x3C);

		RX_BASE_PTR1	= (RALINK_FRAME_ENGINE_BASE+RAPDMA_OFFSET+0x40);
		RX_MAX_CNT1	= (RALINK_FRAME_ENGINE_BASE+RAPDMA_OFFSET+0x44);
		RX_CALC_IDX1	= (RALINK_FRAME_ENGINE_BASE+RAPDMA_OFFSET+0x48);
		RX_DRX_IDX1	= (RALINK_FRAME_ENGINE_BASE+RAPDMA_OFFSET+0x4C);

		/* 6. Counter and Meter Table */
		PPE_AC_BCNT0	= (RALINK_FRAME_ENGINE_BASE+RACMTABLE_OFFSET+0x000); /* PPE Accounting Group 0 Byte Cnt */
		PPE_AC_PCNT0	= (RALINK_FRAME_ENGINE_BASE+RACMTABLE_OFFSET+0x004); /* PPE Accounting Group 0 Packet Cnt */
		/* 0 ~ 63 */

		PPE_MTR_CNT0	= (RALINK_FRAME_ENGINE_BASE+RACMTABLE_OFFSET+0x200); /* 0 ~ 63 */
		/* skip... */
		PPE_MTR_CNT63	= (RALINK_FRAME_ENGINE_BASE+RACMTABLE_OFFSET+0x2FC);

		GDMA_TX_GBCNT0	= (RALINK_FRAME_ENGINE_BASE+RACMTABLE_OFFSET+0x300); /* Transmit good byte cnt for GEport */
		GDMA_TX_GPCNT0	= (RALINK_FRAME_ENGINE_BASE+RACMTABLE_OFFSET+0x304); /* Transmit good pkt cnt for GEport */
		GDMA_TX_SKIPCNT0= (RALINK_FRAME_ENGINE_BASE+RACMTABLE_OFFSET+0x308); /* Transmit skip cnt for GEport */
		GDMA_TX_COLCNT0	= (RALINK_FRAME_ENGINE_BASE+RACMTABLE_OFFSET+0x30C); /* Transmit collision cnt for GEport */

		/* update these address mapping to fit data sheet v0.26, by bobtseng, 2007.6.14 */
		GDMA_RX_GBCNT0	= (RALINK_FRAME_ENGINE_BASE+RACMTABLE_OFFSET+0x320);
		GDMA_RX_GPCNT0	= (RALINK_FRAME_ENGINE_BASE+RACMTABLE_OFFSET+0x324);
		GDMA_RX_OERCNT0	= (RALINK_FRAME_ENGINE_BASE+RACMTABLE_OFFSET+0x328);
		GDMA_RX_FERCNT0 = (RALINK_FRAME_ENGINE_BASE+RACMTABLE_OFFSET+0x32C);
		GDMA_RX_SERCNT0	= (RALINK_FRAME_ENGINE_BASE+RACMTABLE_OFFSET+0x330);
		GDMA_RX_LERCNT0	= (RALINK_FRAME_ENGINE_BASE+RACMTABLE_OFFSET+0x334);
		GDMA_RX_CERCNT0	= (RALINK_FRAME_ENGINE_BASE+RACMTABLE_OFFSET+0x338);
		GDMA_RX_FCCNT1	= (RALINK_FRAME_ENGINE_BASE+RACMTABLE_OFFSET+0x33C);
	}
	FE_INT_ALL = (TX_DONE_INT3 | TX_DONE_INT2 | \
		      TX_DONE_INT1 | TX_DONE_INT0 | \
		      RX_DONE_INT0);
	FE_INT_DLY_INIT = (TX_DLY_INT | RX_DLY_INT);
