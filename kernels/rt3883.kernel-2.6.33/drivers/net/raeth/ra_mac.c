...
void hard_init(struct net_device *dev)
{
	...
	//memset(ei_local, 0 , sizeof(END_DEVICE)); //don't do this in here. it is important. siyou.
...
void dump_cp0(void)
{
	...
	printk("not support dump_cp0() now\n");
/*
...
/* Bouble - 20101209 - 
  * Add /proc/rt3883/sw_port_wifi_pri, to setup each switch port's priority 
  */
#if defined(CONFIG_RTL_TAG_SUPPORT)
#define PROC_SW_PORT_PRI_ENABLE		"sw_port_wifi_pri_enable"
#define PROC_SW_PORT_PRI		"sw_port_wifi_pri"
static struct proc_dir_entry *procSwPortPriEnable;
static struct proc_dir_entry *procSwPortPri;
extern int gSwPortWiFiPriEnable;
extern u_int16_t gSwPortPriMap[];

/* Get current  switch port priority enable/disable setting */
int SwPortPriEnableReadMain(void)
{
	printk("%d\n", gSwPortWiFiPriEnable);
...
/* setup port priority.
  * cmd format: 
  * 	echo "0|1" > /proc/rt3883/sw_port_wifi_pri_enable
  */
static int SwPortPriEnableWriteMain(struct file *file, const char *buffer, unsigned long count, void *data)
{
	int enable=-1;
	...
	if ( enable == 0 || enable == 1 )
		gSwPortWiFiPriEnable=enable;
	else
		printk("sw_port_wifi_pri_enable value doesn't match.\n");
	return count;
}

/* Get current  port priority setting */
int SwPortPriReadMain(void)
{
	int i;
	
	printk("SwitchPortID		map2Wifi_Priority\n");
	for ( i =0; i<= 7; i++ )
	{
		printk("%d			%d\n", i, gSwPortPriMap[i]);
	}
...
/* setup port priority.
  * cmd format: 
  * 	echo "portid prioriry" > /proc/rt3883/sw_port_wifi_pri
  * portid: 0 -7
  * priority: 0 - 7
  */
static int SwPortPriWriteMain(struct file *file, const char *buffer, unsigned long count, void *data)
{
	int port=-1;
	int priority=-1;

	sscanf(buffer,"%d %d", &port, &priority);

	if ( port >=0 && port <= 7  &&  priority >=0  && priority <= 7 )
		gSwPortPriMap[port]=priority;
	else
		printk("Port range: 0-7\nPriority range: 0-7\n");
	return count;
}
#endif
...
#endif
/* Bouble - 20101209 - 
  * Add /proc/rt3883/sw_port_pri, to setup each switch port's priority 
  */
#if defined(CONFIG_RTL_TAG_SUPPORT)
    if ((procSwPortPriEnable = create_proc_entry(PROC_SW_PORT_PRI_ENABLE, 0, procRegDir))){
	 procSwPortPriEnable->read_proc = (read_proc_t*)&SwPortPriEnableReadMain;
	 procSwPortPriEnable->write_proc = (write_proc_t*)&SwPortPriEnableWriteMain;
	}

    if ((procSwPortPri = create_proc_entry(PROC_SW_PORT_PRI, 0, procRegDir))){
	 procSwPortPri->read_proc = (read_proc_t*)&SwPortPriReadMain;
	 procSwPortPri->write_proc = (write_proc_t*)&SwPortPriWriteMain;
	}
#endif
