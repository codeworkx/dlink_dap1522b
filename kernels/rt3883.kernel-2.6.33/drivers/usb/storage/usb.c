			 ...
			 const struct usb_device_id *id)
{
	...
	{
		unsigned char *ptn = intf->cur_altsetting->string;
		int len;
		if(ptn)
		{
			len = strlen(intf->cur_altsetting->string);
			if((ptn[len+1]&0xff)==0x33 && (ptn[len+2]&0xff)==0x80)
			{
				printk("ignore usb storage===>pass to shareport\n");
				return -ENXIO;
			}
		}
	}
