...
void dev_txq_stats_fold_zero(const struct net_device *dev,
...
static int vlan_dev_zero_stats(struct net_device *dev)
{
	struct net_device_stats *stats = &dev->stats;

	dev_txq_stats_fold_zero(dev, stats);

	if (vlan_dev_info(dev)->vlan_rx_stats) {
		struct vlan_rx_stats *p;
		int i;

		for_each_possible_cpu(i) {
			p = per_cpu_ptr(vlan_dev_info(dev)->vlan_rx_stats, i);
			memset(p,0,sizeof(struct vlan_rx_stats));
		}
...
static int vlan_ioctl_handler(struct net *net, void __user *arg)
{
	...
	case SET_VLAN_FLAG_CMD:
		...
		if(args.vlan_qos == 7 )	
		{
       			printk(KERN_INFO "clearn vlan eth2 tx/rx status \n");
			//changed by tsrites for easy reset TX/RX info to zero
			err = vlan_dev_zero_stats(dev);
			break;
		}
