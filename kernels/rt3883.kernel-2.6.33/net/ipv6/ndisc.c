...
static inline int ndisc_is_useropt(struct nd_opt_hdr *opt)
{
	...
	switch(opt->nd_opt_type)//add by rbj for dnssl
	{
		case ND_OPT_RDNSS:
		case ND_OPT_DNSSL:
			return 1;
			  ...
			  int router, int solicited, int override, int inc_opt)
{
	...
	//ifp should be checked here, it may be null (tom, 20100706)
	if(ifp != NULL && ifp->idev != NULL)
...
static void ndisc_router_discovery(struct sk_buff *skb)
{
...
#if defined(ELBOX_USE_IPV6)
	//printk("-RA:\n");
	in6_ntop((u_char *)&ipv6_hdr(skb)->saddr, in6_dev->cnf.ra_saddr, sizeof(in6_dev->cnf.ra_saddr));
	if (ndisc_parse_options(opt, optlen, &ndopts)) {
		...
		if (ndopts.nd_useropts) {
			struct nd_opt_hdr *p;
			u8 buf[(INET6_ADDRSTRLEN+1)* MAX_RA_RDNSS];

			for (p = ndopts.nd_useropts; p;
				p = ndisc_next_useropt(p, ndopts.nd_useropts_end)) {
				struct rdnss_info *rdnssinfo;
				int i, n;
				struct in6_addr *server;

				rdnssinfo = (struct rdnss_info *)p;
				server = (struct in6_addr *)&rdnssinfo->data;
				memset(buf, 0, (INET6_ADDRSTRLEN+1)* MAX_RA_RDNSS);
				for (i = 1; i <= (rdnssinfo->length-1)/2; i++) {
					n = strlen(buf);
					if ( n > (INET6_ADDRSTRLEN+1)* MAX_RA_RDNSS ) {
						sprintf(buf, "%s", "N/A");
						break;
					}
					if ( n > 0 ) n = sprintf(buf, "%s,", buf);
					in6_ntop((u_char *)&server->s6_addr16, buf+n, INET6_ADDRSTRLEN);
					server += 1;
				}
				//printk("[rdnss] server: %s\n", buf);
				sprintf(in6_dev->cnf.ra_rdnss, "%s", buf);
			}
		}
