...
#include <linux/etherdevice.h>//marco
...
#define RESERVE_SESSION 1//marco
...
//add lost include file (tom, 20100728)
#include <net/route.h>
...
//and last 100 only for connection to device from lan
	...
	else if (nf_conntrack_max &&  (nf_conntrack_max-atomic_read(&net->ct.count) <= FIRST_CLASS_RESERVE_SESSION) && (nf_conntrack_max-atomic_read(&net->ct.count) >= SECOND_CLASS_RESERVE_SESSION) )
	{
		...
		{
			goto ALLOC_CONN;//accept connection
		}
		else//drop it
		{
			goto EARLY_DROP;
...
#if defined(CONFIG_RA_HW_NAT) || defined(CONFIG_RA_HW_NAT_MODULE)
	if (nfct_help(ct)) //siyou. add check. nfct_help(ct) maybe NULL.
...
#ifdef CONFIG_ALPHA_FAST_ROUTE//marco
