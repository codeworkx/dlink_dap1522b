...
#define SEMIAUTO_REFRESH_INTERVAL	30
		...
		if( (eth_type == ETH_P_IP) || (eth_type == ETH_P_PPP_SES && ppp_tag == PPP_IP) ) {
			...
			//+++ actually No, pptp will fail in this case. siyou 2011/8/25 10:31
			memset(FOE_INFO_START_ADDR(skb), 0, FOE_INFO_LEN);
			return 1;
			//---
			;
...
void  PpeSetFoeEbl(uint32_t FoeEbl)
{
	...
	if(FoeEbl==1) {
		...
		PpeFlowSet = (BIT_FUC_FOE);//only handle unicast
...
char g_wanif[1024];
char g_lanif[1024];
module_param_string(wan, g_wanif, sizeof(g_wanif), 0);
module_param_string(lan, g_lanif, sizeof(g_lanif), 0);

typedef unsigned long long (*get_hwnat_counters_cb)(char *, int);
extern void register_hwnat_cb(get_hwnat_counters_cb fun1);
extern void unregister_hwnat_cb(void);
#endif
...
#if defined (ALPHA_HW_NAT_STATS)
	register_hwnat_cb(get_hwnat_ctr);
#endif
