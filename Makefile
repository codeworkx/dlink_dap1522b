...
SVNR_TARGET	=	Rules.mk Vars.mk configs tools include comlib build_gpl boards/$(ELBOX_BOARD_NAME) kernels/$(ELBOX_BSP_NAME) \
				templates/$(ELBOX_TEMPLATE_NAME)/progs \
				templates/$(ELBOX_TEMPLATE_NAME)/$(ELBOX_BRAND_NAME)

ifdef NOKERNEL
SVNR_TARGET	=	Rules.mk Vars.mk configs tools include comlib build_gpl boards/$(ELBOX_BOARD_NAME) \
				templates/$(ELBOX_TEMPLATE_NAME)/progs \
				templates/$(ELBOX_TEMPLATE_NAME)/$(ELBOX_BRAND_NAME)
else
SVNR_TARGET	=	Rules.mk Vars.mk configs tools include comlib build_gpl boards/$(ELBOX_BOARD_NAME) kernels/$(ELBOX_BSP_NAME) \
...
SVN_BRANCH_TARGET	=	Makefile Vars.mk Rules.mk configs tools include comlib build_gpl boards/$(ELBOX_BOARD_NAME) kernels/$(ELBOX_BSP_NAME) \
...
model: conf
	...
	$(Q)cp configs/defconfig/wapnd15_dlink_dap1522b_v207_B01.config .config
...
config_clean:
	...
	$(Q)rm -f .config .config.cmd .config.old .defconfig
